...
int main (int argc, char* argv[]) {
    ...
    populate_kmers(files, kmers);

    if (record_kmers) write_rare_abundant_kmers(kmers);

    open_files(files, fhs, counters);
...
int process_reads(string in_path, 
	...
	path = out_path + in_file.substr(0, pos - 1) + ".discards" + in_file.substr(pos);
	...
	if (max_k_freq > 0 && max_lim && abundant_k > max_lim) {
	    counter["abundant_k"]++;
	    retain = false;
...
int populate_kmers(vector<pair<string, string> > &files, 
    ...
    int j = 1;
    for (uint i = 0; i < files.size(); i++) {
	cerr << "Generating kmers from " << j << " of " << files.size() << " [" << files[i].second << "]\n";
...
int open_files(vector<pair<string, string> > &files, 
	...
	path = out_path + files[i].second.substr(0, pos) + suffix;
	fh   = new ofstream(path.c_str(), ifstream::out);
	fhs[files[i].second] = fh;

	if (fhs[files[i].second]->fail()) {
...
int build_file_list(vector<pair<string, string> > &files) {
	...
	while ((direntry = readdir(dir)) != NULL) {
	    ...
	    if (in_file_type == fastq && (suffix == "fq" || suffix == "fastq"))
		files.push_back(make_pair(in_path, file));
	    else if (in_file_type == fasta && (suffix == "fa" || suffix == "fasta"))
		files.push_back(make_pair(in_path, file));
	}
	...
	for (uint i = 0; i < in_files.size(); i++) {
	    ...
	    //
 	    file    = in_files[i];
	    pos     = file.find_last_of("/");
	    in_path = file.substr(0, pos + 1);
	    files.push_back(make_pair(in_path, file.substr(pos+1)));
	}
    }
...
int parse_command_line(int argc, char* argv[]) {
	...
	c = getopt_long(argc, argv, "hvRDkI::K:F:G:M:m:i:y:f:o:t:p:", long_options, &option_index);
...
void help() {
    std::cerr << "kmer_filter " << VERSION << "\n"
              << "kmer_filter [-f in_file_1 [-f in_file_2...] | -p in_dir] -o out_dir [-i type] [-y type] [-D] [-h]\n"
