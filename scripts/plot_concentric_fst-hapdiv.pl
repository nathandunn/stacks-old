...
sub populate_fst {
	...
	while ($line = <$fh>) {
	    ...
	    next if ($ftype eq "hapdiv" && !defined($orgs{$org}->{$parts[2]}));

	    next if ($ftype ne "hapdiv" && !defined($orgs{$org}->{$parts[4]}));
...
sub generate_pdf {
    ...
    #
    # Draw key for Haplotype diversity coloratoin.
    #
    $color_type = "purple";
	...
	foreach $chr (@chrs) {
	    ...
	    draw_fst_pdf($img, $fst->{$key}, $chr, $radius, $type);
	}
...
sub draw_fst_pdf {
    my ($img, $fsts, $chr, $radius, $file_type) = @_;
	...
	if ($file_type eq "hapdiv") {
	    ($r, $g, $b) = hsv2rgb(scale_color($fst), 1.0, 1.0);
	    # ($r, $g, $b) = hsv2rgb(0.61, scale_hapdiv_color($fst), 0.95);
	    # ($r, $g, $b) = hsv2rgb(0.083, scale_hapdiv_color($fst), 1.0);
	    # ($r, $g, $b) = hsv2rgb(0.833, scale_hapdiv_color($fst), 1.0);
	} else {
...
sub draw_hapdiv_key_pdf {
    ...
    # for ($i = -1; $i <= 1; $i += .0133) {
    # 	if ($i >= 0) {
    # 	    ($r, $g, $b) = hsv2rgb(0.61, scale_hapdiv_color($i), 0.95);
    # 	} else {
    # 	    ($r, $g, $b) = hsv2rgb(0.083, scale_hapdiv_color($i), 1.0);
    # 	    # ($r, $g, $b) = hsv2rgb(0.833, scale_hapdiv_color($i), 1.0);
    # 	}

    # 	$color = color2hex($r, $g, $b);

    # 	# print "I: $i; scaled: ", scale_hapdiv_color($i), "\n";
